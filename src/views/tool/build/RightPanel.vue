...
// 处理标签宽度变化，确保不为 NaN
function handleLabelWidthChange(val) {
  if (isNaN(val) || val === null || val === undefined) {
    props.activeData.labelWidth = 100 // 设置默认值
  } else {
    props.activeData.labelWidth = Math.max(0, Math.min(500, val)) // 限制范围
  }
}

// 处理最小行数变化，确保不为 NaN
function handleMinRowsChange(val) {
  if (isNaN(val) || val === null || val === undefined || val < 1) {
    props.activeData.autosize.minRows = 1 // 设置默认值
  } else {
    props.activeData.autosize.minRows = Math.max(1, Math.min(20, val))
    // 确保最小行数不大于最大行数
    if (props.activeData.autosize.maxRows < props.activeData.autosize.minRows) {
      props.activeData.autosize.maxRows = props.activeData.autosize.minRows
    }
  }
}

// 处理最大行数变化，确保不为 NaN
function handleMaxRowsChange(val) {
  if (isNaN(val) || val === null || val === undefined || val < 1) {
    props.activeData.autosize.maxRows = 4 // 设置默认值
  } else {
    props.activeData.autosize.maxRows = Math.max(1, Math.min(20, val))
    // 确保最大行数不小于最小行数
    if (props.activeData.autosize.minRows > props.activeData.autosize.maxRows) {
      props.activeData.autosize.minRows = props.activeData.autosize.maxRows
    }
  }
}
</script>
